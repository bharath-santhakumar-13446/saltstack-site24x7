...
install-agent:
  cmd.run:
    ...
    - name: sudo ./{{ pillar['site24x7']['installfile']['fileName'] }} -i -key={{ pillar['site24x7']['user']['devicekey'] }} -installer=saltstack -f -automation=true
    {% else %}
    - name: sudo ./{{ pillar['site24x7']['installfile']['fileName'] }} -i -key={{ pillar['site24x7']['user']['devicekey'] }} -installer=saltstack -automation=true -proxy={{ pillar['site24x7']['user']['proxy'] }} -f
    {% endif %}
    - user: root
{% else %}
cleanup-install:
